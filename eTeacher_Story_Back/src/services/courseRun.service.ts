...
export class CourseRunService {
    ...
    static async findCourseRun(playerId: string) {
        return AppDataSource.transaction(async (manager) => {
            const courseRunRepo = manager.getRepository(CourseRun);
            ...
            const courseRun = await courseRunRepo.findOne({
                ...
                relations: ['course', 'player', 'gamesSession', 'gamesSession.pupilsState'],
            });

            if (!courseRun) throw new Error("No active course run found for this player");
